...
namespace Repository
{
    public interface IClientRepository
    {
        ...
        Task<bool> CheckClientByNameAsync(GetClientListRequest request, IDbConnection _dbConnection);
        Task<string> CheckAddressAsync(CreateClientRequest request, IDbConnection _dbConnection);
    ...
    public class ClientRepository : IClientRepository
    {
        ...
        public async Task<bool> CheckClientByNameAsync(GetClientListRequest request, IDbConnection _dbConnection)
        {
            string sql = $@"
			            ...
			            where c.[client_name] = @SearchKeyWord;
            ";
        ...
        public async Task<string> CheckAddressAsync(CreateClientRequest request, IDbConnection _dbConnection)
        {
            string sql = $@"DECLARE @tempId INT = 0
                            SELECT @tempId = parent_id FROM general.tb_d_dictionary WITH(NOLOCK) WHERE dictionary_id = {request.CityId ?? 0} AND category = 'City'
                            IF @tempId = 0 OR @tempId <> {request.StateOrProvinceId ?? 0}
                            BEGIN                 
	                            SELECT 'Invalid CityId';
	                            RETURN;
                            END
                            SET @tempId = 0
                            SELECT @tempId = parent_id FROM general.tb_d_dictionary WITH(NOLOCK) WHERE dictionary_id = {request.StateOrProvinceId ?? 0} AND category = 'StateOrProvince'
                            IF @tempId = 0 OR @tempId <> {request.CountryId ?? 0}
                            BEGIN 
	                            SELECT 'Invalid StateOrProvinceId';
	                            RETURN;
                            END
                            SET @tempId = 0
                            SELECT @tempId = parent_id FROM general.tb_d_dictionary WITH(NOLOCK) WHERE dictionary_id = {request.CountryId ?? 0} AND category = 'Country'
                            IF @tempId = 0
                            BEGIN
	                            SELECT 'Invalid CountryId';
	                            RETURN;
                            END
                            SELECT '';";

            CommandDefinition commandDefinition = new CommandDefinition(sql, commandType: CommandType.Text, cancellationToken: default);
            ...
            string sql = $@"
                            ...
                            BEGIN
                                ...
                                (
                                    ...
                                    Union
                                    SElECT client_id from client.tb_cbi_client_basic_information c with(nolock) where c.identity_code LIKE '%'+@SearchKeyWord+'%'
                                    Union
