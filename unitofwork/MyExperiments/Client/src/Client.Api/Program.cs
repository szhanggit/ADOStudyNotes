...
using ServiceClient.Api.Swagger;
using Services.Core;
...
builder.Services.ConfigurePipe();
//builder.Services.ConfigureMediateR();
...
builder.Services.AddScoped<IDammAlgorithm, DammAlgorithm>();
builder.Services.AddScoped<ISecurityKeyService, SecurityKeyService>();

builder.Services.Configure<TxcSwaggerConfiguration>(options => configuration.GetSection("SwaggerConfiguration").Bind(options));
...
builder.Services.RegisterFilter();
...
{
    // for REST
    //options.ListenAnyIP(8005, o => o.Protocols =
    //    HttpProtocols.Http1AndHttp2);

    // for gRPC
    options.ListenAnyIP(9005, o => o.Protocols =
        HttpProtocols.Http2);
});
