...
namespace Services.Tests
{
    ...
    public class ClientRepositoryTest
    {
        ...
        [Fact]
        public async Task CheckClientByNameAsync_HappyPath_ShallReturnSuccess()
        {
            IEnumerable<ClientListItem> clientListItems = new List<ClientListItem> { new ClientListItem { ClientId = 1 } };            
            GetClientListRequest getClientListRequest = new GetClientListRequest { SearchKeyword = "asdf" };
            _dopperOperationMock.Setup(p => p.ProcessSql<SelectMany<ClientListItem>, IEnumerable<ClientListItem>>(It.IsAny<IDbConnection>(), It.IsAny<CommandDefinition>())).ReturnsAsync(clientListItems);
            bool result = await _clientRepository.CheckClientByNameAsync(getClientListRequest, _idbConnectionMock.Object);
            Assert.True(result);
        }

        [Fact]
        public async Task Check_Address()
        {
            CreateClientRequest createClientRequest = new CreateClientRequest { };
            _dopperOperationMock.Setup(p => p.ProcessSql<ExecuteCommandWithReturn<string>, string>(It.IsAny<IDbConnection>(), It.IsAny<CommandDefinition>())).ReturnsAsync("Invalid CountryId");
            string ret = await _clientRepository.CheckAddressAsync(createClientRequest, _idbConnectionMock.Object);
            Assert.Equal("Invalid CountryId", ret);
        }
    }
}
