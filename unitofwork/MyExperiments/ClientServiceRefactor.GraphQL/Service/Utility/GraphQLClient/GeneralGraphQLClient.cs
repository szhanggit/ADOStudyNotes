...
{
    public interface IGeneralGraphQLClient : ITXCGraphqlClientWithHeader
    {
        IGraphQLClient GetGraphQLClient(int tenantId);
    }

    [ExcludeFromCodeCoverageAttribute]
    public class GeneralGraphQLClient : IGeneralGraphQLClient
    {
        private readonly IConfiguration _configuration;
        public GeneralGraphQLClient(IConfiguration configuration)
        {
        ...
        public IGraphQLClient GetGraphQLClient(int tenantId)
        {
            ...
            {
                var svcClient = _configuration["ServiceUrlConfiguration:ServiceDictionaryUrl"];
                if (!String.IsNullOrWhiteSpace(svcClient))
                {
                    _graphQLHttpClient = new GraphQLHttpClient(_configuration["ServiceUrlConfiguration:ServiceDictionaryUrl"], new NewtonsoftJsonSerializer());
