...
namespace Repository
{
    ...
    public class ClientRepository : IClientRepository
    {
        ...
        public async Task<string> GenerateClientIdentityAsync(GenerateClientIdentityCodeModel request, IDbConnection _dbConnection)
        {
            ...
            string dbResult = await _dapperOperation.ProcessSql<ExecuteCommandWithReturn<string>, string>(_dbConnection, commandDefinition);
            if (dbResult != null)
            {
                dbResult = string.Concat(request.TenantId, dbResult);
            }
            return dbResult;
        }
